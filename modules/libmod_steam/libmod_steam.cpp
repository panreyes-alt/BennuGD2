...
#include "libmod_steam_exports.h"
#endif
*/
...
*/
    ...
    static int64_t libmodsteam_achievement_unlock(INSTANCE *my, int64_t *params) {
        if(!steam_loaded) {
            return -1;
        }

        const char *achievement = reinterpret_cast<const char*>(string_get(params[0]));
...
/*
    static int64_t libmodsteam_achievement_delete(INSTANCE *my, int64_t *params) {
        if(!steam_loaded) {
            return -1;
        }
...
/*
    static int64_t libmodsteam_achievement_check(INSTANCE *my, int64_t *params) {
        if(!steam_loaded) {
            return -1;
        }
...
*/
	...
	/*
    static int64_t libmodsteam_achievement_get_icon(INSTANCE *my, int64_t *params) {
		return -1;
		// FIXME
        if(!steam_loaded) {
            return -1;
...
/*
    static int64_t libmodsteam_avatar_get(INSTANCE *my, int64_t *params) {
		return -1;
...
/*
    static int64_t libmodsteam_friend_count(INSTANCE *my, int64_t *params) {
...
#if defined(_WIN32) && defined(__GNUC__)
        int64_t friend_count = SteamAPI_ISteamFriends_GetFriendCount((intptr_t) SteamFriends(),
                                                                 k_EFriendFlagImmediate);
#else
        int64_t friend_count = SteamFriends()->GetFriendCount(k_EFriendFlagImmediate);
#endif
...
/*
    static int64_t libmodsteam_friend_name(INSTANCE *my, int64_t *params) {
        // Get a friend's name
        if(!steam_loaded) {
            return -1;
        }
        ...
        int64_t str_id = string_new(name);
        string_use(str_id);
        return str_id;
    }
*/
...
/*
    static int64_t libmodsteam_friend_nickname(INSTANCE *my, int64_t *params) {
...
/*
    static int64_t libmodsteam_friend_avatar(INSTANCE *my, int64_t *params) {
		return -1;
...
*/
    ...
    static int64_t libmodsteam_lang_get(INSTANCE *my, int64_t *params) {
        // Returns a string with the lang the game should be run in
        const unsigned char *lang = reinterpret_cast<const unsigned char*>(SteamApps()->GetCurrentGameLanguage());

        int64_t str_id = string_new(lang);
        string_use(str_id);
    ...
    static int64_t libmodsteam_is_subscribed_app(INSTANCE *my, int64_t *params) {
    ...
    static int64_t libmodsteam_init(INSTANCE *my, int64_t *params) {
        if(steam_loaded) {
            return 1;
        }
        ...
        const unsigned char *name = reinterpret_cast<const unsigned char*>(SteamFriends()->GetPersonaName());
        int64_t string_id = string_new(name);
        string_use(string_id);
        GLODWORD(libmod_steam, STEAM_USERNAME) = string_id;
...
void  __bgdexport( libmod_steam, module_initialize )() {
	// pass
}

    /* ---------------------------------------------------------------------- */
void  __bgdexport( libmod_steam, module_finalize )() {
	if(!steam_loaded) {
		return;
	}

	SteamAPI_Shutdown();
	string_discard(GLODWORD(libmod_steam, STEAM_USERNAME));
}
